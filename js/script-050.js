function fluidSimulation(canvas) {
    ...
    on(window, 'ego-fluid-auth-state', () => {
        /* Immediately seed the new validation colour into the current fluid. */
        for (let i = 0; i < pointers.length; i++)
            pointers[i].color = generateColor();
        virtualColor = generateColor();
        lastVColorTime = Date.now();

        /* A compact colour wave makes the state change visible immediately. */
        for (let i = 0; i < 8; i++)
            splatStack.push(4 + parseInt(Math.random() * 5));
    });
    ...
    function generateColor() {
        /* Username live-validation colour mode:
           neutral = original cyan/blue/violet/magenta
           valid   = emerald/green
           invalid = red/orange-red */
        const authState = String(window.__EGO_FLUID_AUTH_STATE || 'neutral');
        let h;
        if (authState === 'valid') {
            h = 0.34 + Math.random() * 0.075;      // green → emerald
        } else if (authState === 'invalid') {
            h = Math.random() * 0.028;             // red → red-orange
        } else {
            h = 0.5 + Math.random() * 0.42;        // original palette
        }

        let c = HSVtoRGB(h, authState === 'neutral' ? 0.95 : 0.92, 1.0);
        c.r *= 0.92;
        c.g *= 0.92;
        c.b *= 0.92;
        return c;
...
(function(){
  ...
  function gateVisible(){
    ...
    return !gate.classList.contains('is-hidden') &&
           getComputedStyle(gate).display!=='none' &&
           gate.getBoundingClientRect().width>0 &&
           gate.getBoundingClientRect().height>0;
  }

  function rebuildCanvas(){
    const old=document.getElementById('egoLoginFluidCanvas');
    if(!old)return null;
    try{ if(typeof destroyFluid==='function')destroyFluid(); }catch(e){}
    destroyFluid=null;

    const fresh=old.cloneNode(false);
    fresh.removeAttribute('width');
    fresh.removeAttribute('height');
    fresh.dataset.fluidStarted='';
    old.replaceWith(fresh);
    return fresh;
  }

  function startEgoLoginFluid(force=false){
    if(!gateVisible())return;

    let canvas=document.getElementById('egoLoginFluidCanvas');
    if(!canvas)return;

    if(force || canvas.dataset.fluidStarted==='1'){
    ...
    if(rect.width<2 || rect.height<2){
      requestAnimationFrame(()=>startEgoLoginFluid(force));
      return;
    }

    canvas.dataset.fluidStarted='1';
    canvas.width=Math.max(2,Math.round(rect.width*window.devicePixelRatio));
    canvas.height=Math.max(2,Math.round(rect.height*window.devicePixelRatio));

    try{
      destroyFluid=fluidSimulation(canvas);
    }catch(err){
      console.error('Login fluid background:',err);
      canvas.style.background='#04050c';
    }
  }

  function restartWhenLoginReturns(){
    requestAnimationFrame(()=>{
      requestAnimationFrame(()=>{
        if(gateVisible())startEgoLoginFluid(true);
      });
    });
  }
  ...
  function init(){
    ...
    if(gateVisible())startEgoLoginFluid(false);

    /* Login hides the gate. Logout removes is-hidden.
       Re-create the WebGL context after the gate becomes visible again,
       because browsers may resize/suspend a hidden canvas to 0x0. */
    const observer=new MutationObserver(mutations=>{
      if(mutations.some(m=>m.type==='attributes'&&m.attributeName==='class')){
        if(gateVisible())restartWhenLoginReturns();
      }
    });
    ...
    window.addEventListener('resize',()=>{
      if(gateVisible()){
        clearTimeout(window.__egoFluidResizeTimer);
        window.__egoFluidResizeTimer=setTimeout(()=>startEgoLoginFluid(true),120);
      }
    });

    document.addEventListener('visibilitychange',()=>{
      if(document.visibilityState==='visible'&&gateVisible())restartWhenLoginReturns();
    });
    window.addEventListener('ego-login-gate-shown',restartWhenLoginReturns);
  }
